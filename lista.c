...
void ordenarXnombre( Lista lista) {
    ...
    for (int i = 0; i < lista->tamanio; i++) {
        ...
        for (int j = 0; j < -1+lista->tamanio; j++) {

                EdificioP ubicacion=*((EdificioP*)actual->data);
                EdificioP siguiente=*((EdificioP*)actual->siguiente->data);

            if (strcmp(getNombre(ubicacion),getNombre(siguiente))==1){

                aux=actual->data;
                actual->data=actual->siguiente->data;
                actual->siguiente->data=aux;
...
Nodo actual;
    ...
    for (int j = 0; j < tam; j++) {
            ...
            if (strcmp(getNombre(ubicacion),busco)==0){
                pos=j;
                j=tam;
...
int remover (Lista lista, int pos, void *backup) {
    ...
    proximoCpy = f->siguiente->siguiente; // 'proximoCpy' aponta para o elemento posterior ao que será removido
    memcpy(backup, f->siguiente->data, lista->tamDelDatoEnBytes);

    free(f->siguiente->data); // Remove o VALOR do elemento (Célula) entre 'f' e 'proximoCpy', que pertence justamente o elemento que o usuário deseja remover
    free(f->siguiente); // Remove o ELEMENTO entre 'f' e 'proximoCpy', que é justamente o elemento que o usuário deseja remover
